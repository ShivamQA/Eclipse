...
public class BaseUi {
	...
	protected String executeJavascript(String script, Object object) {
		return ((JavascriptExecutor) driver).executeScript(script, object).toString();
	}
	
	protected Object executeJavascriptObject(String script, Object object) {
	...
	protected void scrollDownRefstag() {
		String element = "document.querySelector('.refs-header-label h2')";
		((JavascriptExecutor) driver).executeScript(element+".scrollIntoView();");

	}
	protected void scrollDownRefpubs() {
		String element = "document.querySelector('.NLM_notes~ul[class=\\\"anchors\\\"]+h2')";
		((JavascriptExecutor) driver).executeScript(element+".scrollIntoView();");

	}
	protected String VerifyReference(String element) {
		
		String elem = (String) ((JavascriptExecutor) driver).executeScript("return "+element+".innerHTML;");
		return elem;
	}
	...
	protected void scrollToTop() {
		((JavascriptExecutor) driver).executeScript("window.scrollBy(0,-100000)");
	}
...
//		wait.waitForPageToLoadCompletely();
	...
	protected String getTimeout(String ...elementToken) {
		  String[] locator = getELementFromFile(this.pageName, elementToken[0]);
		  String time = locator[1]+getProperty("timeout");
		   return time;
		 }
